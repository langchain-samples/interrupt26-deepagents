...
def _read_python_requirement(pyproject_path="pyproject.toml"):
    """Read `requires-python` from pyproject.toml using stdlib only.

    Returns the spec string (e.g. '>=3.11') or None if not found.
    """
    ...
    try:
        match = re.search(r'requires-python\s*=\s*["\']([^"\']+)["\']', p.read_text())
        return match.group(1) if match else None
    except Exception:
        return None


def _check_python_against_spec(py_version, spec):
    """Check py_version (sys.version_info) against a spec like '>=3.11,<3.14'.

    Returns (ok, issues_list). Regex-based so no `packaging` dep is needed.
    Handles >=, >, <=, <, ==, != at major.minor granularity.
    """
    issues = []
    py_tuple = (py_version.major, py_version.minor)
    py_str = f"{py_version.major}.{py_version.minor}"
    for clause in spec.split(","):
        m = re.match(r'(>=|<=|==|!=|>|<)\s*(\d+)\.(\d+)', clause.strip())
        if not m:
            continue
        op, maj, mn = m.group(1), int(m.group(2)), int(m.group(3))
        target = (maj, mn)
        if op == ">=" and py_tuple < target:
            issues.append(f"⚠️  Python {py_str} is below minimum {maj}.{mn}")
        elif op == ">" and py_tuple <= target:
            issues.append(f"⚠️  Python {py_str} must be greater than {maj}.{mn}")
        elif op == "<" and py_tuple >= target:
            issues.append(f"⚠️  Python {py_str} must be less than {maj}.{mn}")
        elif op == "<=" and py_tuple > target:
            issues.append(f"⚠️  Python {py_str} must be at most {maj}.{mn}")
        elif op == "==" and py_tuple != target:
            issues.append(f"⚠️  Python {py_str} must be exactly {maj}.{mn}")
        elif op == "!=" and py_tuple == target:
            issues.append(f"⚠️  Python {py_str} must not be {maj}.{mn}")
...
def check_python_executable_and_version():
    ...
    # Check Python version against pyproject.toml's requires-python spec
    py_spec = _read_python_requirement()
    if py_spec is None:
        print("ℹ️  Could not read requires-python from pyproject.toml — skipping version check")
    else:
        ok, version_issues = _check_python_against_spec(py_version, py_spec)
        if ok:
            print(f"✅ Python version {py_version_str} satisfies requires-python: {py_spec}")
        else:
            issues.extend(version_issues)
            issues.append(f"   pyproject.toml requires-python: {py_spec}")
...
if __name__ == "__main__":
    ...
    # Load environment variables from .env file. override=True matches the
    # workshop's runtime behavior (utils/models.py loads .env with override=True),
    # so this script's view of env vars matches what the notebook/agent will see.
